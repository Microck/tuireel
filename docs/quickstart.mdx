...
<Note>
  ...
  Install it with `brew install ffmpeg` (macOS), `apt install ffmpeg` (Ubuntu), or download from [ffmpeg.org](https://ffmpeg.org).
</Note>
...
The `polished` preset applies the Catppuccin theme, enables cursor overlay, HUD, and sound effects — all in one line.

...
const ASSETS_DIR = resolve(__dirname, "..", "..", "assets", "sounds");
...
describe("buildAudioMixArgs", () => {
  it("produces expected structure with no events", () => {
  ...
  it("adds sound file inputs and adelay entries for each event", () => {
    const events = [
    ...
    const inputCount = inputArgs.filter((argument) => argument === "-i").length;
  ...
  it("clamps event time to total duration", () => {
  ...
  it("includes volume and rate parameters in filter chain", () => {
  ...
  it("uses click and key defaults for corresponding event types", () => {
    const events = [
    ...
    expect(inputArgs).toContain(resolve(ASSETS_DIR, "click-1.mp3"));
    expect(inputArgs).toContain(resolve(ASSETS_DIR, "key-1.mp3"));
  });

  it("respects custom sfx config when resolving sound inputs", () => {
    const events = [{ type: "click" as const, timeMs: 0 }];
    const { inputArgs } = buildAudioMixArgs("input.mp4", events, 1, { click: 3 });

    expect(inputArgs).toContain(resolve(ASSETS_DIR, "click-3.mp3"));
  });
});
...
describe("extractSoundEvents", () => {
  it("maps timeline events into ffmpeg-ready sound events", () => {
    const soundEvents = extractSoundEvents({
      events: [
        { type: "key", frameIndex: 10, timeMs: 333.3 },
        { type: "click", frameIndex: 42, timeMs: 1400 },
      ],
    });

    expect(soundEvents).toEqual([
      { type: "key", timeMs: 333.3 },
      { type: "click", timeMs: 1400 },
    ]);
  });
});

describe("sound config schema", () => {
  it("accepts valid sound effects presets", () => {
  ...
  it("rejects invalid sound preset values", () => {
    const result = configSchema.safeParse({
      steps: [{ type: "launch", command: "echo sound" }],
      sound: {
        effects: {
